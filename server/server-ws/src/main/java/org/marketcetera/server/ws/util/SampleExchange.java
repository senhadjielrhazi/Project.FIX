 ...
 * @since 2.0.0
 * @version $Id: SampleExchange.java 16645 2013-07-27 13:01:43Z colin $
 */
    ...
    implements Application
{
    ...
    public void toAdmin
        (Message message,
         SessionID sessionId)
    {
        add(new MessageEvent(sessionId,message));
    }
	...
	public static void main(String[] args)
	{
	    ...
	    SLF4JLoggerProxy.info(SampleExchange.class,"Exchange is starting");
	    ...
	    try {
	        ...
	        try {
	            SLF4JLoggerProxy.error
	                (SampleExchange.class,t,"Exchange is terminating due to an error");
	        } catch (Throwable t2) {
	    ...
	    SLF4JLoggerProxy.info(SampleExchange.class,
	    ...
	    Runtime.getRuntime().addShutdownHook(new Thread() {
	        ...
	        public void run() {
	            exchange.stop();
	            SLF4JLoggerProxy.info(SampleExchange.class,"Exchange is terminated");
	        }
	    });
	
	    try {
	    	exchange.startWaitingForever();
	    } catch (Throwable t) {
	        try {
	            SLF4JLoggerProxy.error
	                (SampleExchange.class,t,"Exchange is terminating due to an error");
