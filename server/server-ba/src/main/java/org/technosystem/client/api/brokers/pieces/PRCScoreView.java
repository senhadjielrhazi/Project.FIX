...
@RequiresFullAccess
public class PRCScoreView  extends JFrame implements IStrategy {
    ...
    public void refresh(Period period, DataView dataView) throws JFException {
        ...
        HashMap<Instrument, Double> CCYScoreIni = new HashMap<>();
        for (Instrument key : m_Instruments){     
            IBar bidkey0 = history.getBar(key, period, OfferSide.BID, 0);
            IBar askkey0 = history.getBar(key, period, OfferSide.ASK, 0);         
            double keyClose0 = (bidkey0.getClose() + askkey0.getClose()) / 2.0;
            IBar bidkey1 = history.getBar(key, period, OfferSide.BID, (int)(dataView.getPeriod().getInterval()/period.getInterval()));
            IBar askkey1 = history.getBar(key, period, OfferSide.ASK, (int)(dataView.getPeriod().getInterval()/period.getInterval()));         
            double keyClose1 = (bidkey1.getClose() + askkey1.getClose()) / 2.0;
            
            CCYScoreIni.put(key, (keyClose0-keyClose1)/keyClose1);
        }
        ...
        for (Instrument key : CCYScoreIni.keySet()){
            ...
            dataView.getCCYs().put(ccy0, dataView.getCCYs().get(ccy0)+keyScore);
            
            String ccy1 = key.getSecondaryJFCurrency().getSymbol();
            dataView.getCCYs().put(ccy1, dataView.getCCYs().get(ccy1)-keyScore);
        }
        ...
        for(int k = 0; k < dataView.getCCYs().keySet().size(); k++){
            ...
            dataView.getValueList().get(k).setText(""+Math.round(100.0*maxData.getValue()*100.0)/100.0);
        }
    }
