...
import org.marketcetera.trading.arbitrage.forex.BrokerEntry;
...
import org.marketcetera.trading.arbitrage.forex.MarketEntry;
...
public class ArbStrategyView extends Strategy {
	 ...
	 */
	private final static double TRADE_BIDASK_MULT = 0.25;//0.65
	
	/**
	 * The EMA slippage multiplier
	 */
	private final static double TRADE_SLIPPAGE_MULT = 3.0;//2.25
    ...
    public void onStart()
    {
		...
		//Add currency arb entries
    	String symDir , symRev;
    	for(IBrokerEntry entryA:getDataExch().values()){
    		if(entryA.getInstrument() instanceof Currency){
    			Currency ccyA = (Currency)entryA.getInstrument();
	    		for(IBrokerEntry entryB:getDataExch().values()){
	    			if(entryB.getInstrument() instanceof Currency){
	    				Currency ccyB = (Currency)entryB.getInstrument();
	    				String ccyA_R = ccyA.getRightCCY();
	        			String ccyA_L = ccyA.getLeftCCY(); 
	        			String ccyB_R = ccyB.getRightCCY();
	        			String ccyB_L = ccyB.getLeftCCY();
	    				
	        			//Dir-Dir
	        			if(ccyA_R.equals(ccyB_L) && !ccyA_L.equals(ccyB_R)){
	        				symDir = ccyA_L + "/" + ccyB_R;
	        				symRev = ccyB_R + "/" + ccyA_L;
	        				if(!getMarketArbs().containsKey(symDir) && 
	        						!getMarketArbs().containsKey(symRev)){
	        					getWatchList().put(symDir, new FieldEntry(symDir));
	        					getMarketArbs().put(symDir, new MarketEntry(ccyA_L, ccyB_R, getDataExch().values(), TRADE_TIME_LAP));
	        				}
	        			}
	        			 
	        			//Ind-Ind
	        			if(ccyA_L.equals(ccyB_R) && !ccyA_R.equals(ccyB_L)){
	        				symDir = ccyB_L + "/" + ccyA_R;
	        				symRev = ccyA_R + "/" + ccyB_L;
	        				if(!getMarketArbs().containsKey(symDir) && 
	        						!getMarketArbs().containsKey(symRev) ){
	        					getWatchList().put(symDir, new FieldEntry(symDir));
	        					getMarketArbs().put(symDir, new MarketEntry(ccyB_L, ccyA_R, getDataExch().values(), TRADE_TIME_LAP));
	        				}
	        			}
	        			 
	        			//Dir-Ind
	        			if(ccyA_R.equals(ccyB_R) && !ccyA_L.equals(ccyB_L)){
	        				symDir = ccyA_L + "/" + ccyB_L;
	        				symRev = ccyB_L + "/" + ccyA_L;
	        				if(!getMarketArbs().containsKey(symDir) && 
	        						!getMarketArbs().containsKey(symRev) ){
	        					getWatchList().put(symDir, new FieldEntry(symDir));
	        					getMarketArbs().put(symDir, new MarketEntry(ccyA_L, ccyB_L, getDataExch().values(), TRADE_TIME_LAP));
	        				}
	        			}
	        			 
	        			//Ind-Dir
	        			if(ccyA_L.equals(ccyB_L) && !ccyA_R.equals(ccyB_R)){
	        				symDir = ccyA_R + "/" + ccyB_R;
	        				symRev = ccyB_R + "/" + ccyA_R;
	        				if(!getMarketArbs().containsKey(symDir) && 
	        						!getMarketArbs().containsKey(symRev) ){
	        					getWatchList().put(symDir, new FieldEntry(symDir));
	        					getMarketArbs().put(symDir, new MarketEntry(ccyA_R, ccyB_R, getDataExch().values(), TRADE_TIME_LAP));
	        				}
	        			}
	    			}
	        	}
		...
		if(entry != null)
		{
			...
			for(Entry<String, IMarketEntry> marketEntry:getMarketArbs().entrySet()){
				...
				if(field != null){
					...
					if(bid.getPrice() > ask.getPrice() && ask.getPrice() != 0){
