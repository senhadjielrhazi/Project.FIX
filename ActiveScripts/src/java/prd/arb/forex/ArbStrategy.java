...
public class ArbStrategy extends Strategy {
	 ...
	 */
	private final static double TRADE_BIDASK_MULT = 0.50;//0.25/0.05
	
	/**
	 * The EMA slippage multiplier
	 */
	private final static double TRADE_SLIPPAGE_MULT = 3.0;//3.0
	
	/**
	 * The minimum pnl when trading opt mode
	 */
	private final static double TRADE_PNL_OPTRUN = 0.0001;//0.00006
	
	/**
	 * The minimum pnl when trading pos mode
	 */
	private final static double TRADE_PNL_POSRUN = 0.00003;//0.00002/-0.0015
	 ...
	 */
	private final static double TRADE_MAX_POSNB = 10;//4
	 ...
	 */
	private final static double TRADE_MAX_SIZE = 50000.;
	
	/**
	 * The maximum size of a single trade in USD
	 */
	private final static String TRADE_CCY_REF = "USD";
	 ...
	 */
	private final static String STRATEGY_SUFFIX = "ms";
	
	/**
	 * Cache of standard size position
	 */
	private final Map<String, Double> mSizeCache = Maps.newHashMap();
	...
	/**
	 * Cache of all instrument sizes
	 * 
	 * @return the map
	 */
	private Map<String, Double> getSizeCache()
	{
		return mSizeCache;
	}
    ...
    private void tradePosRun() 
    {
    	...
    	for(Entry<DataReferenceKey, ITradeEntry> entryA:getTradeExch().entrySet()){
    		List<ITradeSide> tradesA = entryA.getValue().getTrades();
    		if(!tradesA.isEmpty()){
	    		ITradeSide tradeA = tradesA.get(0);
	    		DataReferenceKey dataAkey = entryA.getKey();
	    		...
	    		for(Entry<DataReferenceKey, ITradeEntry> entryB:getTradeExch().entrySet()){
	    			...
	    			if(!tradesB.isEmpty()){
						...
						IMarketSide bid = tradeA.isBuy()?market.getBid(tradeSz, dataAkey, tradeA.getSide()):market.getBid(tradeSz, dataBkey, tradeB.getSide());
						IMarketSide ask = tradeA.isBuy()?market.getAsk(tradeSz, dataBkey, tradeB.getSide()):market.getAsk(tradeSz, dataAkey, tradeA.getSide());
				...
				if(trade.isOpened()){
					if(trade.isBuy()){
						...
						if(((trade.getMarket().getScore() > TRADE_SCORE_THRESHOLD)  || //go Long
								(order.getPrice().doubleValue() < ask.getPrice()*(1 - TRADE_STOPLOSS_THRESHOLD)))
								&& ask.getSize() >= tradeSz)//check stop loss
						{
							...
							warn("Move trade to market-Long: " + order.getOrderID() + "; score: " + Math.round(trade.getMarket().getScore()*100.)/100. + "; " + order.getPrice() + " vs " + ask.getPrice());
    					}
					}else{
						IMarketSide bid = trade.getMarket().getBid();
						if(((trade.getMarket().getScore() < 1 - TRADE_SCORE_THRESHOLD) || //go short
								(order.getPrice().doubleValue() >bid.getPrice()*(1 + TRADE_STOPLOSS_THRESHOLD)))
								&& bid.getSize() >= tradeSz)//check stop loss
						{
							...
							warn("Move trade to market-Short: " + order.getOrderID() + "; score: " + Math.round(trade.getMarket().getScore()*100.)/100. + "; " + order.getPrice() + " vs " + bid.getPrice());
    					}
					}
					...
					if(trade.isBuy())
					{
						...
						if(ask.getSize() >= tradeSz)
						{
							holder.remove(trade);
							opsTrades.addAll(ask.execute(tradeSz, getIDSuffix()));
							warn("Rejected Re-Entry Long: " + order.getOrderID() + "; score: " + Math.round(trade.getMarket().getScore()*100.)/100. + "; " + order.getPrice() + " vs " + ask.getPrice());
						}
					}else{
						IMarketSide bid = trade.getMarket().getBid();
						if(bid.getSize() >= tradeSz)
						{
							holder.remove(trade);
							opsTrades.addAll(bid.execute(tradeSz, getIDSuffix()));
							warn("Rejected Re-Entry Short: " + order.getOrderID() + "; score: " + Math.round(trade.getMarket().getScore()*100.)/100. + "; " + order.getPrice() + " vs " + bid.getPrice());
						}
					}
				}
			}
	...
	private double getStandardSize(String symbol) 
	{
		...
		if(tradeSz == null){
			if(symbol.contains("/")){
				String ccy = symbol.split("/")[0];
				
				if(ccy.equals(TRADE_CCY_REF)){
					tradeSz = TRADE_MAX_SIZE;
				}else{
					IMarketEntry ccyMarket = getMarketArbs().get(TRADE_CCY_REF + "/" + ccy);
					if(ccyMarket != null){
						tradeSz = TRADE_MAX_SIZE * ccyMarket.getBid().getPrice();
						getSizeCache().put(symbol, tradeSz);
					}else{
						ccyMarket = getMarketArbs().get(ccy + "/" + TRADE_CCY_REF);
						if(ccyMarket != null){
							tradeSz = TRADE_MAX_SIZE / ccyMarket.getBid().getPrice();
							getSizeCache().put(symbol, tradeSz);
						}else{
							tradeSz = TRADE_MAX_SIZE;
							warn("The following symbol: " + symbol + ", has no USD normalised size, default size is: " + TRADE_MAX_SIZE);
						}
					}
				}
			}else{
				...
				if(instMarket != null){
					tradeSz = TRADE_MAX_SIZE / instMarket.getAsk().getPrice();
					getSizeCache().put(symbol, tradeSz);
				}else{
					tradeSz = TRADE_MAX_SIZE;
					warn("The following instrument: " + symbol + ", has no nominal value size, default size is: " + TRADE_MAX_SIZE);
				}
			}
		}
    ...
    public void onCallback(Object inData) 
    {
    	...
    	} else if(inData.equals(Reason.SYNC_TRADES))
    	{
    		...
    		Map<BrokerID, Integer> positions = Maps.newHashMap();
    		for(ITradeEntry holder:getTradeExch().values()){
    			...
    			int position = positions.getOrDefault(holder.getBrokerID(), 0) + holder.getFilledNB();
    			positions.put(holder.getBrokerID(), position);
